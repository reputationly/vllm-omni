...
def resolve_omni_config(
    ...
    *,
    ...
    # 走到这里意味着模型没有解析出注册管线，只能退到通用扩散默认档。此时 deploy YAML
    # 是按管线拓扑合并的，没有拓扑可合 —— 里面写的并行度、offload、VAE tiling 会被
    # 整份丢掉并悄悄回落默认值。大模型上这会在约 20 分钟后以一个无从解释的 OOM 暴露
    # （我们在 MiniMax-H3 上踩过）。显式传了 --deploy-config 就必须生效，否则报错。
    if deploy_config_path is not None:
        raise ValueError(
            ...
            "pipeline_registry.py::OMNI_PIPELINES), and deploy configs are merged against a "
            "pipeline topology. Register a PipelineConfig for this model, or drop "
            "--deploy-config and pass the equivalent settings as CLI flags."
        )
