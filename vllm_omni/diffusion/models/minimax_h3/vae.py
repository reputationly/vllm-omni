...
class MiniMaxH3VideoVAE(nn.Module, DistributedVaeMixin):
    ...
    def decode_latent(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        # _revert_frames denormalizes in bounded chunks straight into host
        # memory (see below) instead of upstream's one-shot revert_tensor, so
        # a 15s 4K decode does not need 2x its full-resolution size in RAM.
        # It already performs the ndim==4 -> 5d normalization upstream does
        # inline here, so nothing further is needed after it returns.
