...
def _multipart_reference_order(
    ...
    multipart request on the default deployment into a 400.
    """
...
async def _persist_uploaded_media_references(
    ...
    videos: list[str] = []
    audios: list[str] = []
    # The *upload* order as ``(kind, index-within-its-bucket)``. Splitting an
    # ordered list into three buckets is exactly where an order stops existing,
    # and H3 reads the order as semantic -- *video, image* rebuilt as
