...
MINIMAX_H3_FPS = 24
# The audio VAE's rate; standalone references are normalized onto it exactly
# like a reference video's soundtrack.
...
def _prepare_encoder_images(
    ...
    *,
    height: int,
    width: int,
    keyframe_resize_mode: str | None = None,
    reference_image_shape: Callable[[Image.Image, int, int], tuple[int, int]] | None = None,
) -> list[Any]:
    """Resize references to the geometry the deployment's contract pins.

    ``reference_image_shape`` and ``keyframe_resize_mode`` are passed by the
    diffusion pipeline, which owns the resolved contract. Both default to the
    upstream behaviour so an encoder-only stage keeps working unchanged.
    """
    if not images:
        return []
    if task == "ref2va":
        shape = reference_image_shape or (lambda image, _w, _h: resolve_minimax_h3_reference_image_shape(image))
        return [image.resize(shape(image, width, height), Image.Resampling.LANCZOS) for image in images]
    if task != "fl2va":
        return images
    if keyframe_resize_mode is not None:
        # The follower keyframe is cover-cropped under the official contract
        # and stretched under legacy; see keyframes.py.
        return prepare_fl2va_keyframes(images, width=width, height=height, mode=keyframe_resize_mode)
...
def prepare_encoder_inputs(
    ...
    *,
    ...
    images = _prepare_encoder_images(
        ...
        height=height,
        width=width,
        keyframe_resize_mode=getattr(strategy, "fl2va_keyframe_resize_mode", None),
    ...
    if task == "fl2va":
        ...
        video_has_audio = [bool(item["input_has_audio"]) for item in prepared_videos]
        num_audios = 0 if lock_audio else len(audio_values)
        requested_order = multi_modal_data.get("reference_order")
        if strategy is None:
            # Encoder-only stage with no resolved contract: keep the upstream
            # bucket order verbatim.
            audio_index = 0
            ...
            if requested_order and getattr(strategy, "reference_order_mode", None) != MINIMAX_H3_ORDER_REQUEST:
            ...
            # Rebinds rather than extends: the derived order already carries the
            # image labels appended above. See ordered_references.py.
    ...
    if lock_audio:
        ...
        # Admission first, on the clip as submitted: truncating to the generated
        # duration would otherwise turn a 20-second upload into a request that
        # passes the 15-second limit.
        validate_reference_audio_waveforms(standalone_audios)
        if standalone_audios and getattr(strategy, "reference_audio_target_truncation", False):
            # A standalone audio reference is packed exactly like a reference
            # video's soundtrack, so it is normalized exactly like one:
            # truncate at the source rate, upmix, resample once.
            standalone_audios = normalize_standalone_reference_audios(
                ...
                resample_mode=strategy.reference_audio_resample_mode,
            )
