...
> ## ⚠️ 本文已被 2026-08-22 的实测取代，两处结论作废
>
> **接续报告：`docs/minimax-h3-ref2va-剪枝量化全量实测-2026-08-22.md`**（54 次运行，权重血统逐位验过）。
>
> **1）§3 的"剪枝 r8"阶梯数据全部作废——权重取错了分支。**
> 那批 ref2va 剪枝件派生自 `MiniMax-H3-Pruned-r8/transformer`，而那是 **FL2VA** 的权重；
> ref2va 应当用 `transformer_ref`（上游 README 明写）。逐位证据：该产物与官方 FL2VA 原件
> 4/4 相同、与官方 Ref2VA 原件 0/4 相同。**画质与保真结论一律不可用**；显存/耗时结论仍成立
> （两分支架构完全一致）。正确权重的完整包络见接续报告 §5.3。
>
> **2）§1 "二者互补"需要限定条件。**
> 实测：在 **GPU 峰值**上剪枝与 INT8 **互相替代**——只剪枝、只量化、两者都上，
> case1 峰值均为 29555 MiB，一 MiB 不差（峰值由 encode 阶段决定，与 DiT 权重大小脱钩）。
> 互补只体现在**宿主内存**（144.8 → 126.4 → 115.6 GiB）与**磁盘**（43.8 → 37.5 → 19.5 GiB）上。
>
> 其余部分（§4 CONVROT 评估、§7 转换器规则、§9 机器与坑）仍然有效；§6 的执行计划已全部完成。

> 目标读者：下一个接手这个任务的 agent。本文自包含，不依赖对话上下文。
> 前置背景文档：`docs/minimax-h3-剪枝前后对比-2026-08-20.md`（剪枝 r8 的 **FL2VA**/TP4 实测，
> 用的是 `transformer/`，对 FL2VA 而言是正确权重，不受上述勘误影响）。
