...
> **2026-08-20 补充**：本节结论成立，但根因不是"管线强制归桶"，而是**训练上限**。官方在
> [Issue #6](https://github.com/Tencent-Hunyuan/HunyuanImage-3.0/issues/6) 明确回复"trained to support image
> generation up to 1K resolution"；社区实测改 `image_base_size` 会得到"尺寸对了但画面是乱的"，
> 这条路已被证伪，不要再尝试。另外请求层的 `size` 只决定**比例**不决定像素量，且是**静默归桶**
> （要 2048×2048 照常 `200` 返回 1024×1024，无任何警告）。
> 详见 `vLLM-Omni-HunyuanImage3-分辨率天花板与编辑能力选型评测.md` §2。
...
- ~~**步数实验未做**（§7 第 1 条），这是最该补的一块。~~
  **2026-08-20 已补**：8 / 16 / 30 / 50 步扫描，细节只涨 26%、耗时涨 3 倍、SSIM 反降——蒸馏版工作点就是 8 步，此路封闭。
  同轮发现**指令写法的影响远大于步数**（加一句保护主体的描述即 +69% 主体锐度且 SSIM 同时上升）。
  详见 `vLLM-Omni-HunyuanImage3-分辨率天花板与编辑能力选型评测.md` §5。
...
- **INT8 峰值 37429 MiB 是 1024² / ≤3 参考图条件下的。** ~~更大底图或~~第 4 张参考图未压测，8.6% 余量下大概率 OOM。
  **2026-08-20 订正**：**"更大底图"这半句不成立**——参考图进 VAE 前先归桶再 LANCZOS 缩放，8192² 与 1024² 同为 4096 token；
  ViT 走 naflex，`max_num_patches=1024` 封顶。实测 2048² 输入 58s 正常跑完，无 OOM。
  真正会撞线的是**参考图张数**（每张 ≤5120 token，4 张越过 `max_position_embeddings=22800`，此项仍为推算未实测）。
  详见 `vLLM-Omni-HunyuanImage3-分辨率天花板与编辑能力选型评测.md` §3。
