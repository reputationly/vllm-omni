...
FL2VA 三个生产实例统一使用上游关键帧几何：首帧决定输出画布，异比例尾帧保持比例放大后居中裁剪。该选择独立于完整 Diffusers contract，不改变 RNG、默认帧数或 Ref2VA 行为。
...
实现约束：`match` 的面积基准必须是**本次请求的输出画布** `width × height`。
固定写成最大画布 `768 × 1344` 只在该分辨率下等价；对 832×480 等档位会保留约
2.6 倍的参考图 rows，既偏离蒸馏训练预处理，也浪费编码与去噪资源。共用 Ref2VA
profile 保留 Base 既有的固定面积策略；只有模型分区带 `distilled` 元数据时，pipeline
才自动把实例策略切到 `match`。因此 Base 与 Turbo 的行为相互独立。

## 9. LoRA 离线融合倍率审计

2026-08-19 对三份 NFS 正式候选重新执行独立融合验算。每份均覆盖 312 对 LoRA A/B、
208 个 native target 和 10,063,872 个确定性 BF16 采样值；三份最大误差都是 0：

| 产物 | rank / alpha | 有效倍率 | 发生变化的采样值 | LoRA SHA256 |
|---|---:|---:|---:|---|
| FL2VA Turbo4 768p | 128 / 128 | 1.0 | 2,960,753 | `1bdabc2e…7416c` |
| FL2VA Turbo8 | 128 / 8 | 0.0625 | 510,048 | `e16ac208…807f` |
| Ref2VA Turbo4 | 128 / 8 | 0.0625 | 701,293 | `9e642fc8…1ae99` |

组装工具现已把这项验算变成强制前置条件，并把完整来源 SHA、倍率和采样摘要写进
`_minimax_h3.distilled`；官方单卡 harness 也会从 checkpoint 读取 alpha，显式传入的
值若与 metadata 不同则直接失败。离线融合与运行时 LoRA 在线性代数上等价，剩余差异
只有写回 BF16 的舍入；本次验算证明当前三份产物就是该 BF16 公式的实际结果。
